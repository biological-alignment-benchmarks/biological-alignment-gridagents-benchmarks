...
def run_episode(hparams: dict = {}):
    ...
    if env_params.get('env_type') == 'zoo':
        ...
        if hparams.get('env_entry_point') is not None:
            gym.envs.register(
                id=env_params['name'],
                entry_point=hparams['env_entry_point'], # e.g. 'aintelope.environments.savanna_gym:SavannaGymEnv'
                kwargs={'env_params': env_params}
            )
        env = gym.make(hparams['env'])
        obs_size = env.observation_space.shape[0]
    else:
        print(f'env_type {env_params.get("env_type")} not implemented. Choose: [zoo, gym]')
    ...
    for i in range(warm_start_steps):
        ...
        if done:
            if verbose: 
                print(f'Uhoh! Your agent terminated during warmup on step {i}/{warm_start_steps}')
            break
    ...
    while done is False:
        ...
        reward, done = agent.play_step(net, epsilon, hparams.get('device', 'cuda'))
        episode_reward += reward
        if render_mode is not None:
            env.render(render_mode)
            
    if verbose:
        print(f'Simple Episode Evaluation completed. Final episode reward: {episode_reward}')