...
class AsciiRenderState:
    def __init__(self, agent_states, grass_patches, settings):
        ...
        self.scene = Scene((self.window_size, self.window_size), is_main_scene=True, clear_char='.')
        ...
        for agent_name, agent_pos in agent_states.items():
            agent_image = """x""".replace('x', self.ascii_symbols['agent'][count % len(self.ascii_symbols['agent'])])
            agent_sprite = Sprite((agent_pos[0], self.window_size - agent_pos[1]), agent_image, layer=1)
        ...
        for grass_pos in grass_patches: #np.argwhere(grass_patches==1):
            ...
            grass_sprite = Sprite((grass_pos[0], self.window_size - grass_pos[1]), grass_image, colour=tc.GREEN, layer=2)
    ...
    def render(self, agent_states, grass_patches):
        for agent_name, agent_pos in agent_states.items():
            ...
            move_y = (self.window_size - agent_pos[1]) - (self.window_size - prev_pos[1])
        ...
        for grass_pos in np.argwhere(grass_patches==1):
            grass_image = """x""".replace('x', self.ascii_symbols['food'])
            grass_sprite = Sprite((grass_pos[0], grass_pos[1]), grass_image, colour=tc.GREEN, layer=2)
            self.grass_sprites.append(grass_sprite)
