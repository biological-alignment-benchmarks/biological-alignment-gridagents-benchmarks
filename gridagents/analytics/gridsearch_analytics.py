...
from gridagents.analytics.plotting import save_plot, maximise_plot
from gridagents.config.config_utils import register_resolvers
from gridagents.utils import wait_for_enter, try_df_to_csv_write, RobustProgressBar
...
@hydra.main(
    version_base=None,
    config_path=os.path.join(os.getcwd(), "gridagents", "config"),
...
def gridsearch_analytics(cfg: DictConfig) -> None:
    ...
    initial_config_gridsearch = OmegaConf.load(
        os.path.join("gridagents", "config", gridsearch_config_file)
    )
    ...
    if create_specialised_pipeline_config_with_best_parameters:
        ...
        pipeline_config = OmegaConf.load(
            os.path.join("gridagents", "config", pipeline_config_file)
        )
        ...
        for combination_i in range(0, return_n_best_combinations):
            ...
            OmegaConf.save(
                ...
                os.path.join("gridagents", "config", specialised_pipeline_config_file),
                resolve=False,
            )
...
def copy_log_folder(experiment_name, source_uuid, source_base, dest_base, cfg):
    ...
    if False:  # TODO: function parameter
        source_code_archive = os.path.join(
            source_pipeline_log, "gridagents_code_archive.zip"
        )
